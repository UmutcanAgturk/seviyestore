...
final class StockCountsRestController extends AbstractRestController
{
    ...
    public function registerRoutes(): void
    {
        register_rest_route(RestApiRegistrar::NAMESPACE, '/depo/stock-counts', [
            [
                'methods' => 'GET',
                'callback' => [$this, 'index'],
                'permission_callback' => $this->requireCapability(WarehouseCapability::MANAGE_STOCK_COUNTS->value),
            ],
            [
                'methods' => 'POST',
                'callback' => [$this, 'store'],
    ...
    public function index(WP_REST_Request $request): WP_REST_Response
    {
        ...
        $counts = array_map($this->serialize(...), $this->stockCounts->all($status));

        return new WP_REST_Response($counts);
    }

    public function store(): WP_REST_Response
    {
        $levels = $this->currentManagedStockLevels();

        if ($levels === []) {
            $message = __('Stok takibi açık hiçbir ürün bulunamadı.', 'seviye-depo');

            return new WP_REST_Response(['message' => $message], 422);
        }

        $stockCount = $this->stockCounts->open($levels, get_current_user_id());
     ...
     * yapılan kalemler tutuluyor (aksi halde expected_quantity anlamsız).
     *
     * @return array<int, int> productId => stok miktarı
     */
    private function currentManagedStockLevels(): array
    {
